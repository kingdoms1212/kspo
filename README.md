...
- 프로그램 현황의 검색·정렬·요일 조건, 중복 제거, 페이지 이동, 예산 범위와 비교표를 개선했습니다.
- 전국 시·도 지도를 시군구까지 확인할 수 있도록 공통 모듈화하고, 프로그램 현황을 서울 구 단위 지역 분포로 구성했습니다.
- 원본 CSV 4종을 서울 자료로 정제하고 지역 코드 품질을 검증하는 배치와 Manifest·로그·세대 관리를 구현했습니다.
- 서버 기동 및 APScheduler 예약 배치, 프로세스별 CSV 캐시 자동 갱신, CSV·DB 저장 방식과 서울·전국 전환 구조를 구현했습니다.
- 데이터 최신화 관리 모달과 진행률·실행 상태·변경 내역, 서비스 이용 가이드를 추가했습니다.
