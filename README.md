...
단위·렌더링 테스트 89개를 통과했습니다. 실제 CSV를 사용하는 Django Client에서도 세 화면, 프로그램 2페이지·복합 필터, 시설 상세 및 시설 2페이지, 엑셀 3종에서 200을 확인했습니다. 루트 302도 회귀 테스트합니다. 별도의 TypeScript 검사·프런트 빌드 설정은 없습니다.
...
| 파일 | 사용 모듈 | 적재 |
|---|---|---|
| `스포츠강좌이용권 이용현황 정보.csv` | 대시보드 | 원장 517,101행을 시군구 단위로 집계 |
| `공공체육시설 프로그램 정보.csv` | 프로그램 | 396,693행 중 중복 101,660행 제외 → 295,033건 |
| `전국체육시설현황 데이터.csv` | 시설 | 152,967행 중 삭제 표시 13,843행 제외 → 139,124건 (공공 35,673 · 신고 102,881 · 등록 570) |
| `체육시설 인접 대중교통 정보.csv` | 시설 상세 | 1,639,279행 · 시설 상세를 열 때만 색인 |
