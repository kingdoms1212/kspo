...
프로그램 정보 입력 후 `AI 검토` 버튼으로 `POST /dashboard/ai/plan`을 호출합니다.
다이얼로그에서 결과를 확인하고 `레포트에 포함하기`를 체크한 경우에만 서버가 서명한
검토 토큰을 `POST /dashboard/plan/preview`에 전달합니다. 보고서 생성은 AI를 재호출하지 않습니다.
토큰은 입력값·선택 시설에 연결되고 1시간 후 만료됩니다. 입력 변경·서명 변조 시 포함을 거부합니다.
실패한 검토에는 토큰을 발급하지 않습니다. 보고서 인쇄·최근 5건 복원에는 포함 당시 결과가 유지됩니다.

메인 통계 카드 아래 `AI 현황 해석`은 `POST /dashboard/ai/region`으로 별도 실행됩니다.
서울 선택 구의 이용권 통계만 보내 요약·특징·기획 확인사항을 반환합니다. 버튼 클릭 때만 호출하며
다른 메인 기능을 막지 않습니다. 결과에서 기존 프로그램 설계 흐름으로 이동합니다.
지역/CSV 세대/통계/모델/모드/분석 기준 버전을 키로 24시간 파일 캐시를 유지합니다.
캐시는 `main/.ai-cache`에 최대 200건이며 Git에 포함하지 않습니다. 배포 재시작 등으로 파일이
사라지면 다음 요청에서 다시 생성됩니다. 실패 응답은 캐시하지 않습니다.
동시 AI 요청 제한은 기존과 같이 프로세스당 1건이며 여러 워커 사이의 중복 호출 방지는 아닙니다.
