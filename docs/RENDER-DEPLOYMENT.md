...
2. `data/`에 원본 CSV 4개가 이미 있고 비어 있지 않으면 압축 해제를 생략한다. 없거나 빈 파일만 압축의 `origin/`에서 스트리밍으로 해제한다. 기존 파일을 사용했는지 빌드 로그에 표시한다.
...
새 원본을 적용하려면 `origin.zip`을 갱신하고 원본 CSV가 없는 깨끗한 빌드로 재배포한다.
기존 원본 CSV도 ZIP 내용으로 덮어쓰려면 `python main/manage.py prepare_render_data --force-extract`를 실행한다.
예약 배치는 배포된 원본을 다시 가공하며 외부에서 새로운 원본을 다운로드하지 않는다.
원본 CSV가 모두 있으면 ZIP 없이도 빌드할 수 있다. 원본이 부족한데 ZIP도 없거나 배치 검증에 실패하면 빌드가 실패한다.
압축 해제를 생략해도 지역 CSV와 SHA-256 manifest는 빌드마다 다시 생성한다.
